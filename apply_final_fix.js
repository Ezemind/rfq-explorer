...
async function fixFunctionFinal() {
  try {
    console.log('🔧 Creating final corrected get_or_create_ai_control function...');
...
RETURNS TABLE(
  ...
  ai_enabled BOOLEAN, 
  ai_disabled_at TIMESTAMP, 
  last_human_message_at TIMESTAMP, 
  auto_reenable_hours INTEGER
) AS $$
BEGIN
    ...
    SELECT 
      ...
      ac.ai_enabled AS ai_enabled, 
      ac.ai_disabled_at AS ai_disabled_at, 
      ac.last_human_message_at AS last_human_message_at, 
      ac.auto_reenable_hours AS auto_reenable_hours
    FROM ai_controls ac
